...
class TestSSDP(TestBase):
    packet_args: List = list(packet_generator())
    byte_packets: List = [SSDPDatagram("M-SEARCH", **p).encode() for p in packet_args]
    ...
    async def test_m_search_reply_unicast(self) -> NoReturn:
        replies: OrderedDict = {(self.query_packet.encode(), ("10.0.0.1", 1900)): self.reply_packet.encode()}
        sent: List = []

        with mock_tcp_and_udp(self.loop, udp_replies=replies, udp_expected_addr="10.0.0.1", sent_udp_packets=sent):
            reply = await m_search(
                "10.0.0.2", "10.0.0.1", self.successful_args, timeout=1, loop=self.loop, unicast=True, ignored=None
            )

        self.assertEqual(reply.encode(), self.reply_packet.encode())
        self.assertListEqual(sent, [self.query_packet.encode()])

        with self.assertRaises(UPnPError):
            with mock_tcp_and_udp(self.loop, udp_expected_addr="10.0.0.1", udp_replies=replies):
                await m_search(
                    "10.0.0.2", "10.0.0.1", self.successful_args, timeout=1, loop=self.loop, unicast=False, ignored=None
                )

    async def test_m_search_reply_multicast(self) -> NoReturn:
        replies: OrderedDict = {(self.query_packet.encode(), (SSDP_IP_ADDRESS, 1900)): self.reply_packet.encode()}
        sent: List = []

        with mock_tcp_and_udp(self.loop, udp_replies=replies, udp_expected_addr="10.0.0.1", sent_udp_packets=sent):
            reply = await m_search(
                "10.0.0.2", "10.0.0.1", self.successful_args, timeout=1, loop=self.loop, ignored=None
            )
    ...
    async def test_packets_sent_fuzzy_m_search(self) -> NoReturn:
        sent: List = []
    ...
    async def test_packets_fuzzy_m_search(self) -> NoReturn:
        replies: OrderedDict = {(self.query_packet.encode(), (SSDP_IP_ADDRESS, 1900)): self.reply_packet.encode()}
        sent: List = []
